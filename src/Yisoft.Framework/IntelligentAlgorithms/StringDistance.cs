	...
	public class StringDistance
	{
				...
				for (var j = 1; j < m2; j++)
				{
					...
					else
					{
						...
						if (allowTransposition && i > 1
							...
							_num[i, j] = Math.Min(_num[i, j], _num[i - 2, j - 2] + weight(Choice<T>.Transpose(strOld[i - 1], strOld[i - 2])));
					}
				}
			}
		...
		public int LongestCommonSubstring(string str1, string str2)
		{
			return string.IsNullOrEmpty(str1) || string.IsNullOrEmpty(str2)
				? 0
				: LongestCommonSubstring(str1.ToCharArray(), str2.ToCharArray(), out int _, out int __);
		}
		...
		{
			var length = LongestCommonSubstring(sliceOld, sliceNew, out int posOld, out int posNew, comparer);
		...
		private void _TryDiff<T>(Slice<T> sliceOld, Slice<T> sliceNew, IEqualityComparer<T> comparer, List<DiffPair<T>> result)
		{
			...
			{
				_DiffPrivate(sliceOld, sliceNew, comparer, result);
			}
